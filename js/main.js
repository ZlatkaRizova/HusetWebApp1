function getData () {
    fetch("http://veselinvelkov.com/mywpsite/wp-json/wp/v2/posts?_embed")
    .then(res=>res.json())
    .then(showPosts)
}

function getAllPostsByTag(id){
    fetch("http://veselinvelkov.com/mywpsite/wp-json/wp/v2/posts?_embed&tags="+id)
    .then(res=>res.json())
    .then(showPosts)
}
...
function getMenu(){
    fetch("http://veselinvelkov.com/mywpsite/wp-json/wp/v2/tags")   //to decide if I need tags or categories and tochange it
    .then(e=>e.json())
    .then(showMenu)
}
...
function showMenu(tags){
    ...
    tags.forEach(function(tag){
        if(tag.count > 0){
          ...
          clone.querySelector("a").setAttribute("href", "template.html?tagid="+tag.id)
...
//        here we change stuff:
       ...
       link.setAttribute("href", "webapp.html?id="+thePost.id);
